...
| 2 | `TASKS-2.md`（当前轮） · `TASKS.md`（第一轮，已完成） | 执行计划与验收标准 |
| 3 | **实际代码**（有测试覆盖的） | 21 个 Go 包测试全绿，是可信资产 |
| 4 | `docs/01`~`docs/16` | 设计意图，非现状描述 |

### 进度

- **第一轮 T0~T15：已完成并验收**（构建通过、21 包测试全绿、`verify` 退出码 0、魔鬼夹具 Shiki/KaTeX/Mermaid/表格/任务列表/脚注全通过）。四个阻断性问题 B1~B4 已解决。
- **第二轮 R1~R6：见 [TASKS-2.md](../TASKS-2.md)**，核心是 R1 多语言编辑闭环（见下方 A6 行）。
...
| A2 | `docs/08 §1` Tailwind + shadcn/ui | 纯手写 CSS | **待决策**，见 `TASKS-2.md` R6（建议只上 Tailwind） | R6 |
| A3 | `docs/08 §1` 按 feature 分目录 | 16 个组件挤在 63 行单文件 | ✅ 已完成（features/components/api 拆分，14 个 feature 目录） | T1 |
| A4 | `docs/08 §4` CodeMirror 编辑器 | ✅ 已实现（含自动保存、localStorage、修订、预览） | **保持** | — |
| A5 | `docs/08 §5` 分类/标签/菜单/友链/多语言矩阵页 | ✅ 已完成：分类/标签（T9）、菜单/友链（T10）、页面（T11）、翻译矩阵 + 批量成本确认（T12）、回收站/日志/文章列表筛选（T13） | **保持** | T9~T13 |
| **A6** | `docs/08 §4.5` 编辑器语言标签栏 + 权威字段锁 | 🔴 **缺失，且原因与先前记录不符**：编辑器把 `locale` 硬编码为 `zh-CN`（`PostEditorPage.tsx:25/43/116`）。后端 `?locale=` **早已支持**（`server.go` 多处读取 query），所以不是「等后端」，是前端没接。另缺 `POST/DELETE /posts/{id}/locales/{locale}` 两个端点（`docs/07 §3.3` 规定未实现） | **按规格改造 —— 产品主线，最高优先级** | R1 |
...
| C2 | `docs/12 §4` AI / 渲染器熔断器 | 🔴 **仍缺失**（T7 未覆盖）。`provider.go` 有重试 `maxRetries=3` + 指数退避 + `Retry-After` 解析，**但无熔断器** —— provider 持续故障时每个任务各自重试 3 次，烧 token 且刷日志 | **按规格改造** | R2 |
| C3 | `docs/12 §3.7` stall 检测 | 🔴 **确认缺失**（`render.go` 中的 `stall` 只是注释词，非实现） | **按规格改造** | R5 |
| C4 | `docs/12 §8` 编辑器三层保存 | 🟡 **部分**：`baseHash` 已在发送（`PostEditorPage.tsx:44`、`PagesPage.tsx:81`），但**前端不处理 409** —— 乐观并发做了一半；另缺离线状态指示 | **按规格改造** | R3 |
