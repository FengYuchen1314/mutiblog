...
export interface BackupTask { id: string; kind: "Backup"; operation: "create" | "import-local" | "import-url" | "restore"; status: "queued" | "running" | "succeeded" | "failed"; backupId?: string; error?: string; createdAt: string; startedAt?: string; completedAt?: string; }
...
export interface ThemeRecord { schemaVersion: number; id: string; name: string; version: string; requires?: string; engine: "react-ssr"; server: string; assets?: string; screenshot?: string; screenshotUrl?: string; settingsSchema?: string; settingsReload?: "rebuild"; postTemplates?: ThemeContentTemplate[]; pageTemplates?: ThemeContentTemplate[]; active: boolean; builtIn: boolean; status: "ready" | "incompatible"; conditions: ThemeCondition[]; }
...
export interface ThemeSettingsSchema { title?: string; "x-i18n"?: Record<string, string>; "x-enum-i18n"?: Record<string, Record<string, string>>; type: "object" | "string" | "boolean" | "number" | "integer"; format?: string; enum?: string[]; default?: unknown; properties?: Record<string, ThemeSettingsSchema>; }
export interface ThemeSettings { themeId: string; active: boolean; schema: ThemeSettingsSchema; values: Record<string, unknown>; }
export interface SiteSettings { sourceLocale: string; adminLocale: "en" | "zh-CN"; timezone: string; baseUrl?: string; activeTheme: string; primaryMenu?: string; idStrategy: "uuid" | "timestamp"; locales: Record<string, { title: string; subtitle?: string; description?: string }>; }
...
export interface PublishResult {
  ...
  translation: { status: "queued" | "not-needed" | "not-configured" | "failed"; taskId?: string };
}
...
export interface TranslationTask {
  ...
  status: "queued" | "running" | "succeeded" | "failed" | "needs-review";
  targets: Array<{ locale: string; expectedRevision: number; status: string; attempts: number; error?: string; completedAt?: string }>;
  createdAt: string;
  startedAt?: string;
  completedAt?: string;
  error?: string;
...
export const api = {
  ...
  rebuildIndex: (csrfToken: string) => request<IndexStats>("/api/v1/admin/index/rebuild", { method: "POST", headers: { "X-CSRF-Token": csrfToken } }),
  ...
  updatePostSettings: (csrfToken: string, id: string, body: { revision: number; categories: string[]; tags: string[]; cover: string; commentPolicy: "open" | "closed"; template: string }) =>
    request<Post>(`/api/v1/admin/posts/${encodeURIComponent(id)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  publishPost: (csrfToken: string, id: string, revision: number) =>
  ...
  changePostStatus: (csrfToken: string, id: string, action: "unpublish" | "recycle" | "restore", revision: number) => request<{ post: Post; build: { status: "succeeded" | "failed" } }>(`/api/v1/admin/posts/${encodeURIComponent(id)}/${action}`, { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  deletePost: (csrfToken: string, id: string, revision: number) => request<void>(`/api/v1/admin/posts/${encodeURIComponent(id)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  ...
  updatePageSettings: (csrfToken: string, id: string, body: { revision: number; cover: string; commentPolicy: "open" | "closed"; template: string }) =>
    request<Post>(`/api/v1/admin/pages/${encodeURIComponent(id)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  publishPage: (csrfToken: string, id: string, revision: number) =>
  ...
  changePageStatus: (csrfToken: string, id: string, action: "unpublish" | "recycle" | "restore", revision: number) => request<{ post: Post; build: { status: "succeeded" | "failed" } }>(`/api/v1/admin/pages/${encodeURIComponent(id)}/${action}`, { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  deletePage: (csrfToken: string, id: string, revision: number) => request<void>(`/api/v1/admin/pages/${encodeURIComponent(id)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  rebuildSite: (csrfToken: string) =>
  ...
  createTaxonomy: (csrfToken: string, kind: "categories" | "tags", body: { id?: string; name: string; description?: string; parentId?: string }) =>
    request<Taxonomy>(`/api/v1/admin/taxonomies/${kind}`, { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateTaxonomyStructure: (csrfToken: string, kind: "categories" | "tags", id: string, revision: number, parentId?: string) => request<Taxonomy>(`/api/v1/admin/taxonomies/${kind}/${encodeURIComponent(id)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision, parentId: parentId ?? "" }) }),
  updateTaxonomyLocale: (csrfToken: string, kind: "categories" | "tags", id: string, locale: string, body: { revision: number; name: string; description?: string; seoTitle?: string; seoDescription?: string }) =>
    request<Taxonomy>(`/api/v1/admin/taxonomies/${kind}/${encodeURIComponent(id)}/locales/${encodeURIComponent(locale)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  deleteTaxonomy: (csrfToken: string, kind: "categories" | "tags", id: string, revision: number) => request<void>(`/api/v1/admin/taxonomies/${kind}/${encodeURIComponent(id)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  attachments: () =>
  ...
  createLinkGroup: (csrfToken: string, body: { id?: string; name: string; description?: string; order?: number }) => request<LinkGroup>("/api/v1/admin/links/groups", { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  createLink: (csrfToken: string, body: { id?: string; groupId: string; url: string; logo?: string; name: string; description?: string; order?: number }) => request<SiteLink>("/api/v1/admin/links/items", { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateLinkGroup: (csrfToken: string, id: string, revision: number, order: number) => request<LinkGroup>(`/api/v1/admin/links/groups/${encodeURIComponent(id)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision, order }) }),
  updateLink: (csrfToken: string, id: string, body: { revision: number; groupId: string; url: string; logo?: string; order: number }) => request<SiteLink>(`/api/v1/admin/links/items/${encodeURIComponent(id)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateLinkGroupLocale: (csrfToken: string, id: string, locale: string, body: { revision: number; name: string; description?: string }) => request<LinkGroup>(`/api/v1/admin/links/groups/${encodeURIComponent(id)}/locales/${encodeURIComponent(locale)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateLinkLocale: (csrfToken: string, id: string, locale: string, body: { revision: number; name: string; description?: string }) => request<SiteLink>(`/api/v1/admin/links/items/${encodeURIComponent(id)}/locales/${encodeURIComponent(locale)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  deleteLinkGroup: (csrfToken: string, id: string, revision: number) => request<void>(`/api/v1/admin/links/groups/${encodeURIComponent(id)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  deleteLink: (csrfToken: string, id: string, revision: number) => request<void>(`/api/v1/admin/links/items/${encodeURIComponent(id)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  menus: () => request<{ items: MenuRecord[] }>("/api/v1/admin/menus"),
  createMenu: (csrfToken: string, body: { id?: string; label: string }) => request<MenuRecord>("/api/v1/admin/menus", { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  addMenuItem: (csrfToken: string, menuId: string, body: { id?: string; parentId?: string; targetKind: "internal" | "external"; url: string; label: string; openInNew: boolean; order: number; revision: number }) => request<MenuRecord>(`/api/v1/admin/menus/${encodeURIComponent(menuId)}/items`, { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateMenuItem: (csrfToken: string, menuId: string, itemId: string, body: { revision: number; parentId?: string; targetKind: "internal" | "external"; url: string; openInNew: boolean; order: number }) => request<MenuRecord>(`/api/v1/admin/menus/${encodeURIComponent(menuId)}/items/${encodeURIComponent(itemId)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateMenuLocale: (csrfToken: string, id: string, locale: string, body: { revision: number; label: string }) => request<MenuRecord>(`/api/v1/admin/menus/${encodeURIComponent(id)}/locales/${encodeURIComponent(locale)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateMenuItemLocale: (csrfToken: string, menuId: string, itemId: string, locale: string, body: { revision: number; label: string }) => request<MenuRecord>(`/api/v1/admin/menus/${encodeURIComponent(menuId)}/items/${encodeURIComponent(itemId)}/locales/${encodeURIComponent(locale)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  deleteMenu: (csrfToken: string, id: string, revision: number) => request<void>(`/api/v1/admin/menus/${encodeURIComponent(id)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  deleteMenuItem: (csrfToken: string, menuId: string, itemId: string, revision: number) => request<MenuRecord>(`/api/v1/admin/menus/${encodeURIComponent(menuId)}/items/${encodeURIComponent(itemId)}`, { method: "DELETE", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ revision }) }),
  ...
  restoreBackup: (csrfToken: string, id: string) => request<{ status: "succeeded"; health: "ready"; report: StaticBuildReport; safetyBackup: BackupRecord; reauthenticate: true }>(`/api/v1/admin/backups/${encodeURIComponent(id)}/restore`, { method: "POST", headers: { "X-CSRF-Token": csrfToken } }),
  ...
  installTheme: (csrfToken: string, file: File) => {
	const body = new FormData();
	body.append("file", file, file.name);
	return request<{ theme: ThemeRecord; build?: StaticBuildReport }>("/api/v1/admin/themes/install", { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body });
  },
  installThemeURL: (csrfToken: string, url: string) => request<{ theme: ThemeRecord; build?: StaticBuildReport }>("/api/v1/admin/themes/install-url", { method: "POST", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ url }) }),
  activateTheme: (csrfToken: string, id: string) => request<{ activeTheme: string; report: StaticBuildReport }>(`/api/v1/admin/themes/${encodeURIComponent(id)}/activate`, { method: "POST", headers: { "X-CSRF-Token": csrfToken } }),
	reloadTheme: (csrfToken: string, id: string) => request<{ theme: ThemeRecord; build?: StaticBuildReport }>(`/api/v1/admin/themes/${encodeURIComponent(id)}/reload`, { method: "POST", headers: { "X-CSRF-Token": csrfToken } }),
  ...
  saveThemeSettings: (csrfToken: string, id: string, values: Record<string, unknown>) => request<{ settings: ThemeSettings; build?: StaticBuildReport }>(`/api/v1/admin/themes/${encodeURIComponent(id)}/settings`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify({ values }) }),
  resetThemeSettings: (csrfToken: string, id: string) => request<{ settings: ThemeSettings; build?: StaticBuildReport }>(`/api/v1/admin/themes/${encodeURIComponent(id)}/settings/reset`, { method: "POST", headers: { "X-CSRF-Token": csrfToken } }),
  settings: () => request<{ site: SiteSettings; comments: CommentSettings }>("/api/v1/admin/settings"),
  updateSiteSettings: (csrfToken: string, body: { baseUrl: string; timezone: string; adminLocale: "en" | "zh-CN"; primaryMenu: string; idStrategy: "uuid" | "timestamp" }) => request<{ site: SiteSettings; build: { status: string } }>("/api/v1/admin/settings/site", { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateSiteLocale: (csrfToken: string, locale: string, body: { title: string; subtitle?: string; description?: string }) => request<{ site: SiteSettings; build: { status: string } }>(`/api/v1/admin/settings/site/locales/${encodeURIComponent(locale)}`, { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
  updateCommentSettings: (csrfToken: string, body: CommentSettings) => request<CommentSettings>("/api/v1/admin/settings/comments", { method: "PUT", headers: { "X-CSRF-Token": csrfToken }, body: JSON.stringify(body) }),
