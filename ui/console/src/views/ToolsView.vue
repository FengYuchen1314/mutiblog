...
import { ApiError, api, createIndexRebuildTaskId, createStaticBuildTaskId, type IndexStats, type StaticBuildReport, type UnifiedTask } from "@/api/client";
...
async function rebuildIndex() {
  ...
  try {
    ...
    try { await api.task(taskId); }
    catch (taskError) { if (taskError instanceof ApiError && taskError.status === 404 && indexTaskId.value === taskId) indexTaskId.value = ""; }
...
async function rebuildSite() {
  ...
  try {
    const result: { status: "succeeded"; report: StaticBuildReport } = await api.rebuildSite(session.session.csrfToken, taskId);
    notice.value = t("toolsPage.siteRebuilt", { count: result.report.files });
  } catch (caught) {
    try { await api.task(taskId); }
    catch (taskError) { if (taskError instanceof ApiError && taskError.status === 404 && siteTaskId.value === taskId) siteTaskId.value = ""; }
...
<template>
  <div class="page">
    ...
    <div class="page-body settings-grid">
      ...
      </div><div v-if="route.query.setupBuildFailed === '1'" class="form-alert">{{ t("login.setupBuildFailed") }}</div><TaskProgress v-if="indexTaskId" :task-id="indexTaskId" @terminal="handleIndexTaskTerminal" /><TaskProgress v-if="siteTaskId" :task-id="siteTaskId" /><div v-if="notice" class="form-success">{{ notice }}</div><div v-if="error" class="form-alert">{{ error }}</div></VCard>
