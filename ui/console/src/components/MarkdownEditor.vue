...
function handleEditorScroll() {
  if (editor) synchronizeScroll(editor.scrollDOM);
}
function handlePaneScroll(event: Event) {
  synchronizeScroll(event.currentTarget as HTMLElement);
}
function synchronizeScroll(source: HTMLElement) {
  ...
  const targets = [
    editor?.scrollDOM,
    ...Array.from(container.value?.querySelectorAll<HTMLElement>(".source-comparison,.markdown-preview") ?? []),
  ];
  for (const target of targets) {
    if (target && target !== source && target.offsetParent !== null)
...
function insertLink(view = editor) {
  ...
  view.dispatch({
    changes: { from: selection.from, to: selection.to, insert: replacement },
    selection: { anchor: selection.from + selected.length + 3 },
  });
  view.focus();
  return true;
}
  ...
  <div ref="container" class="markdown-editor" :class="{ 'markdown-editor--fullscreen': fullscreen }">
    <div class="editor-toolbar">
      <div class="mode-tabs">
        ...
        <button :class="{ active: mode === 'split' }" type="button" @click="mode = 'split'">
      ...
      <button
        ...
        type="button"
        :class="{ active: comparisonOpen }"
        @click="comparisonOpen = !comparisonOpen"
      >
        {{ comparisonOpen ? t("markdownEditor.hideSource") : t("markdownEditor.showSource") }}
      </button>
      <button type="button" @click="fullscreen = !fullscreen">
        {{ fullscreen ? t("markdownEditor.exitFullscreen") : t("markdownEditor.fullscreen") }}
      </button>
    </div>
    <div
      class="editor-panes"
      :class="[`editor-panes--${mode}`, { 'editor-panes--comparison': comparisonOpen && sourceComparison }]"
    >
      <section v-if="comparisonOpen && sourceComparison" class="source-comparison" @scroll="handlePaneScroll">
        <header>
          {{ t("markdownEditor.source", { locale: sourceComparison.locale }) }} · {{ sourceComparison.title }}
        </header>
        <pre>{{ sourceComparison.markdown }}</pre>
      </section>
