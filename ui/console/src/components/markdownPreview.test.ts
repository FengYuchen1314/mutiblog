...
describe("editor Markdown mathematics", () => {
  test("marks inline and display dollar mathematics for the browser-only converter", () => {
    const output = createPreviewMarkdownRenderer().render("Euler: $e^{i\\pi}+1=0$.\n\n$$\\int_0^1 x^2\\,dx=\\frac{1}{3}$$");
  ...
  test("keeps currency, escaped dollars, and code spans out of the MathJax markers", () => {
    const output = createPreviewMarkdownRenderer().render("The range is $20,000 to $30,000. Valid math is $2+2$. Use `$x$` and \\$literal.");
  ...
  test("explicitly rejects dynamic or URL-capable TeX before creating a marker", () => {
    const output = createPreviewMarkdownRenderer().render("$\\href{javascript:alert(1)}{unsafe}$\n\n$$\\require{html} x$$");
