...
func TestCategoryInheritanceAndLazyBody(t *testing.T) {
	...
	categoryYAML := []byte("id: technology\norder: 1\n")
	if err := os.WriteFile(filepath.Join(dataRoot, "categories", "technology.yaml"), categoryYAML, 0o644); err != nil {
		t.Fatal(err)
	}
	categoryYAML = []byte("id: linux\nparent: technology\norder: 2\n")
	if err := os.WriteFile(filepath.Join(dataRoot, "categories", "linux.yaml"), categoryYAML, 0o644); err != nil {
		t.Fatal(err)
	}
