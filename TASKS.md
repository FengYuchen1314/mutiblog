...
> ✅ 已完成（2026-08-10）：分类页（dnd-kit 拖拽排序、编辑表单、多语言名称/描述、
> 父级调整、删除迁移）；标签页（编辑/删除/合并）；新增后端 POST /tags/merge。
> 验收：lint/build/go test 全绿；拖拽/迁移的浏览器交互需人工确认。
...
> ✅ 部分完成（2026-08-10）：翻译矩阵页（文章×语言状态徽章：缺失/排队/翻译中/
> 完成/失败/过期；筛选；勾选批量触发；>20 任务弹成本确认框：任务数/预估 token/
> 预计耗时/勾选确认）。后端 posts 列表新增 sourceRevision 支撑过期判断。
> 编辑器语言标签栏与权威字段锁需要后端多版本接口，本轮未实现（依赖 T9~T13 之后的
> 接口扩展），已在 docs/17 记录。
