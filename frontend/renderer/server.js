...
async function themeMarkup(props, html) {
...
async function render(props) {
  ...
  const body = await markdown(props.body, props.markdown || {})
  const locale = props.locale || 'en'
  const description = props.description
    ? `<meta name="description" content="${esc(props.description)}">`
    : ''
  const theme = props.theme || {}
  const accent = /^#[0-9a-f]{3}(?:[0-9a-f]{3})?$/i.test(theme.accentColor || '')
    ? theme.accentColor
    : '#2563eb'
  const custom = String(theme.customCSS || '')
    .replace(/<\/?style/gi, '')
    .replace(/<script/gi, '')
  ...
  const style =
    '<style>:root{--accent:' +
    esc(accent) +
    '}body{max-width:760px;margin:3rem auto;padding:0 1rem;font-family:system-ui,sans-serif;lin' +
    'e-height:1.7}pre{padding:1rem;background:#0f172a;color:#f8fafc;overflow:auto}code{font-fam' +
    'ily:ui-monospace,monospace}a{color:var(--accent)}.heading-anchor{margin-left:.4rem;text-de' +
    'coration:none;opacity:.55}.heading-anchor:hover{opacity:1}.locale-switcher,.pagination{dis' +
    'play:flex;gap:.4rem;list-style:none;padding:1rem 0}.locale-switcher a,.pagination a,.pagin' +
    'ation span{padding:.2rem .55rem;border:1px solid #cbd5e1;border-radius:.3rem}.pagination [' +
    'aria-current=page],.locale-switcher [aria-current=page]{font-weight:700;background:#e2e8f0' +
    '}.search-input{width:100%;padding:.65rem;font:inherit}.search-results{padding:0;list-style' +
    ':none}.shiki{position:relative;background:#f6f8fa;padding:1rem;overflow:auto}.shiki code{f' +
    'ont-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:.9rem;line-heigh' +
    't:1.6}.shiki span{color:var(--shiki-light)}html.dark .shiki{background:#0f172a}html.dark .' +
    'shiki span{color:var(--shiki-dark)}pre[data-lang]::after{content:attr(data-lang);position:' +
    'absolute;top:.55rem;right:.75rem;font-size:.72rem;color:#64748b;text-transform:uppercase;l' +
    'etter-spacing:.05em;pointer-events:none}pre.with-line-numbers code{counter-reset:line}pre.' +
    'with-line-numbers .line::before{counter-increment:line;content:counter(line);display:inlin' +
    'e-block;width:1.7rem;margin-right:1rem;color:#94a3b8;text-align:right;user-select:none}.me' +
    'rmaid-island{margin:1.2rem 0}.mermaid-source{background:#f8fafc;border:1px solid #e2e8f0;c' +
    'olor:#334155;overflow:auto;white-space:pre-wrap}'
  custom + '</style>'
  const itemLink = (item) => {
  ...
  const pagination =
    props.pagination?.total > 1
      ? '<nav aria-label="Pagination"><ol class="pagination">' +
        props.pagination.links
          .map(
            (link) =>
              '<li>' +
              (link.current
                ? '<span aria-current="page">' + esc(link.page) + '</span>'
                : '<a href="' + esc(link.url) + '">' + esc(link.page) + '</a>') +
              '</li>',
          )
          .join('') +
        '</ol></nav>'
      : ''
  const locales = (props.alternates || [])
    .filter((item) => item?.hrefLang && item.hrefLang !== 'x-default')
    .map(
      (item) =>
        '<a href="' +
        esc(item.href) +
        ...
        esc(item.hrefLang) +
        '"' +
        (item.hrefLang === locale ? ' aria-current="page"' : '') +
        '>' +
        esc(item.hrefLang) +
        '</a>',
    )
  ...
  const localeSwitcher = locales
    ? '<nav class="locale-switcher" aria-label="Languages">' + locales + '</nav>'
    : ''
  let content
  if (props.kind === 'home' || props.kind === 'collection') {
    content =
      '<main>' +
      localeSwitcher +
      '<h1>' +
    ...
    const indexURL = esc(props.searchIndexURL || '/search-index.json')
    content =
      '<main>' +
      localeSwitcher +
      '<h1>' +
      esc(title) +
      '</h1><label for="site-search">' +
      esc(title) +
      '</label><input class="search-input" id="site-search" type="search" autocomplete="off"><ul ' +
      'class="search-results" id="search-results"></ul><script>(function(){const input=document.g' +
      'etElementById("site-search"),results=document.getElementById("search-results");let index;i' +
      'nput.addEventListener("input",async function(){const q=input.value.trim().toLowerCase();if' +
      '(!q){results.replaceChildren();return}if(!index)index=await fetch("' +
      indexURL +
      '").then(r=>r.ok?r.json():[]);const found=index.filter(x=>(x.title+" "+x.description).toLow' +
      'erCase().includes(q)).slice(0,20);results.replaceChildren(...found.map(x=>{const li=docume' +
      'nt.createElement("li"),a=document.createElement("a"),p=document.createElement("p");a.href=' +
      'x.url;a.textContent=x.title;p.textContent=x.description||"";li.append(a,p);return li}))})}' +
      ')()</script></main>'
  } else if (props.kind === 'not_found') {
    content =
      '<main><h1>' +
      esc(title) +
      '</h1><p>' +
      esc(props.message || 'Page not found.') +
      '</p><p><a href="/' +
      esc(props.prefix || String(locale).toLowerCase()) +
      '/">' +
      esc(props.homeLabel || 'Back to home') +
      '</a></p></main>'
  } else {
    content = '<article>' + localeSwitcher + '<h1>' + esc(title) + '</h1>' + body + '</article>'
  }
